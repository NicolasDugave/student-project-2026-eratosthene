$(document).ready(function () {
  ...
  // 1. Define the year array
  const yearSteps = [1907, 1925, 1937, 1973, 1983, 2000, 2025];

// 2. Slider Change Event
  $("#year-slider").on("input", function () {
    const index = $(this).val();
    const map_id = yearSteps[index];

    // Update the big date text in the header
    $("#date-text").html(map_id);

    // 3. Layer Management Logic (Migrated from your old click event)
    for (const key in map_dict) {
      map.removeLayer(map_dict[key]);
      if (bases_dict[key]) map.removeLayer(bases_dict[key]);
      if (b_dict[key]) map.removeLayer(b_dict[key]);
      
      // Remove transport layers if they exist
      if (t_dict[key]) {
        for (const type in t_dict[key]) {
          map.removeLayer(t_dict[key][type]);
        }
      }
      
      // Remove markers and hide articles
      if (markers[key]) markers[key].forEach((m) => map.removeLayer(m));
      if (articles[key]) articles[key].forEach((a) => a.hide());
    }

    // 4. Add new layers based on map_id
    if (b_dict[map_id]) b_dict[map_id].addTo(map);
    if (bases_dict[map_id]) bases_dict[map_id].addTo(map);
    
    // Cumulative transport logic
    Object.entries(t_dict).forEach(([year, layers]) => {
      if (map_id >= year) {
        Object.values(layers).forEach(l => l.addTo(map));
      }
    });

    if (markers[map_id]) markers[map_id].forEach((m) => m.addTo(map));
    if (articles[map_id]) articles[map_id].forEach((a) => a.show());

    // Update Leaflet Control if necessary
    updateLayerControl(map_id);
  });
});
